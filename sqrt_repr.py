...
def plot_sqrt(*args, ax=None, rewrite_y_axis_label=True):
    """
    ...
    rewrite_y_axis_label: Whether to (True) rewrite the ylabels so they reflect counts;
                            or to (False) keep the y label as sqrt(counts).
                            Note that this option comes with the caveat of needing extra flux space.
    """
    assert len(args)<=2, "Call signature = plot_sqrt([boundary_2d], counts, ax=None, rewrite_y_axis_label=False)".format()
    if ax is None:
        ax = plt.subplot()
...
if __name__=='__main__':
    ...
    plot_sqrt(boundaries, counts, ax=axtop, rewrite_y_axis_label=False)
